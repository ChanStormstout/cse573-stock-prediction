...
def main():
    ...
    lines = ['# 实验索引','', '当前入口是 **stock_recency_dense_4h/v5** 与 **stock_reaction_features_4h/v4**；历史目录保留原名称，不能将一小时结果与四小时结果直接比较。reaction v4 是当前已核验的文章级 artifact；其四个候选均未通过完整 promotion gate，因此没有 W0--W3 下游结果。','']
