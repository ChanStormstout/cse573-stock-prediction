...
DEFAULT_PUBLIC = OUT_ROOT / "v2"
DEFAULT_PRIVATE = WORK / "reaction_features_4h" / "v2"
...
def build_pairs(index: pd.DataFrame, private: Path):
    ...
    for n, row in enumerate(index.itertuples(index=False), 1):
        ...
        for symbol in ("AAPL", "AMZN"):
            ...
            candidates.append({"article_key": article_key, "target_pair_key": f"{article_key}|{symbol}", "archive": row.archive, "member": row.member, "symbol": symbol, "title": title, "body": body, "url": str(row.url or ""), "site": str(row.site or ""), "published_utc": pub, "crawled_utc": crawled, "available_utc": available, "language": language, "tier": int(ev["tier"]), "evidence_type": ev["evidence_type"], "evidence_sentence_id": ev.get("sentence_id"), "context_sentence_ids": ev.get("context_sentence_ids", []), "group_key": g, "dedup_basis": gtype, "normalized_hash": str(row.normalized_hash or ""), "exact_hash": str(row.exact_hash or ""), "title_norm": str(row.title_norm or "")})
...
def _load_bars(symbol):
    ...
    b = pd.read_csv(RAW / f"{name}5.csv", header=None,
    ...
    if b.index.duplicated().any():
        raise AssertionError(f"duplicate five-minute bars: {symbol}")
    return b.sort_index()


def _completed_context(bars, session, available, minutes):
    """Return only bars whose *end* is at or before article availability."""
    n = minutes // 5
    usable = bars[(bars.index >= session.open) &
                  (bars.index + pd.Timedelta("5min") <= available) &
                  (bars.index < session.close)]
    idx = usable.index[-n:]
    # The window is anchored at the last completed bar, not at a bar that is
    # still forming.  A contiguous suffix is required; otherwise the feature
    # is explicitly missing.
    valid = (len(idx) == n and len(idx) > 0 and
             np.all(np.diff(idx.view("i8")) == 5 * 60 * 10**9))
    if not valid:
        return {"valid": False, "used_bar_end_utc": None, "return": 0.0, "rv": 0.0}
    return {
        "valid": True,
        "used_bar_end_utc": (idx[-1] + pd.Timedelta("5min")).isoformat(),
        "return": float(np.log(bars.loc[idx[-1], "close"] / bars.loc[idx[0], "open"])),
        "rv": float(np.sqrt(np.square(np.log(bars.loc[idx, "close"].to_numpy() /
                                            bars.loc[idx, "open"].to_numpy())).sum())),
    }


def bars_and_reactions(canonical, private):
    sessions = schedule(); sessions["day"] = sessions.open.dt.strftime("%Y-%m-%d")
    bars = {s: _load_bars(s) for s in ("AAPL", "AMZN")}
    out = []
    horizons = (30, 60, 120, 240)
    for row in canonical.itertuples(index=False):
        available = pd.Timestamp(row.available_utc).tz_convert("UTC")
        eligible = sessions[(sessions.open <= available) & (sessions.close > available)]
        rec = {
            "article_key": row.article_key,
            "target_pair_key": getattr(row, "target_pair_key", f"{row.article_key}|{row.symbol}"),
            "symbol": row.symbol,
        ...
        if len(eligible):
            ...
            rec["session_day"] = session.day
            idx = b.index
            start_pos = int(idx.searchsorted(available, side="left"))
            # Reaction begins at the first bar start at/after availability.
            start = idx[start_pos] if start_pos < len(idx) else None
            for h in horizons:
                n = h // 5
                sl = idx[start_pos:start_pos + n] if start is not None else idx[:0]
                end = (sl[-1] + pd.Timedelta("5min")) if len(sl) else None
                contiguous = len(sl) == n and len(sl) > 0 and np.all(np.diff(sl.view("i8")) == 5 * 60 * 10**9)
                complete = bool(contiguous and start >= available and end <= session.close and
                                not b.loc[sl, ["open", "high", "low", "close"]].isna().any().any())
                rec[f"reaction_{h}m_start_utc"] = start.isoformat() if complete else None
                rec[f"reaction_{h}m_end_utc"] = end.isoformat() if complete else None
                rec[f"reaction_{h}m_matured"] = int(complete)
                rec[f"reaction_{h}m_valid"] = int(complete)
                rec[f"reaction_{h}m"] = float(np.log(b.loc[sl[-1], "close"] / b.loc[sl[0], "open"])) if complete else np.nan
        else:
            for h in horizons:
                rec[f"reaction_{h}m_start_utc"] = None
                rec[f"reaction_{h}m_end_utc"] = None
                rec[f"reaction_{h}m_matured"] = 0
                rec[f"reaction_{h}m_valid"] = 0
                rec[f"reaction_{h}m"] = np.nan
        # Price context is a separate pre-event feature and must use only
        # completed bars.  Keep row-level provenance for every horizon.
        if len(eligible):
            session = eligible.iloc[0]
            for mins in (5, 15, 30, 60):
                c = _completed_context(b, session, available, mins)
                rec[f"pre_{mins}m_return"] = c["return"]
                rec[f"pre_{mins}m_rv"] = c["rv"]
                rec[f"pre_{mins}m_valid"] = int(c["valid"])
                rec[f"pre_{mins}m_used_bar_end_utc"] = c["used_bar_end_utc"]
            ends = [rec[f"pre_{m}m_used_bar_end_utc"] for m in (5, 15, 30, 60) if rec[f"pre_{m}m_used_bar_end_utc"]]
            rec["price_context_max_used_bar_end_utc"] = max(ends) if ends else None
            rec["price_context_complete"] = int(all(rec[f"pre_{m}m_valid"] for m in (5, 15, 30, 60)))
            rec["minutes_from_open"] = float((available - session.open).total_seconds() / 60)
        else:
            for mins in (5, 15, 30, 60):
                rec[f"pre_{mins}m_return"] = 0.0; rec[f"pre_{mins}m_rv"] = 0.0
                rec[f"pre_{mins}m_valid"] = 0; rec[f"pre_{mins}m_used_bar_end_utc"] = None
            rec["price_context_max_used_bar_end_utc"] = None
            rec["price_context_complete"] = 0
            rec["minutes_from_open"] = 0.0
        out.append(rec)
    rx = pd.DataFrame(out)
    rx["available_utc"] = pd.to_datetime(rx.available_utc, utc=True)
    rx.to_pickle(private / "article_reactions_all_time.pkl")
    return rx
...
def audit(public, private, index, cand, canonical, rx, counts, schema_keys, sample_objs):
    ...
    schema={"index_shape":list(index.shape),"index_columns":index.columns.tolist(),"raw_json_top_level_keys":sorted(schema_keys),"sample_schema":sample_objs,"text_field":"text","entities_field":"entities","availability_rule":"max(valid published_utc,crawled_utc)","reaction_start_rule":"first bar start >= availability","reaction_maturity_rule":"reaction bar end <= session close and contiguous","price_context_rule":"only bars with bar_end <= availability","pair_unit":"(article_key,target_symbol)","target_pair_key":"article_key|target_symbol","association_review":"deterministic tier-stratified cards; independent human gold not claimed","public_text_policy":"no article text or evidence sentences published"};(public/"DATA_SCHEMA_AUDIT.md").write_text("# Full-corpus schema audit\n\n```json\n"+json.dumps(schema,indent=2,default=str)+"\n```\n")
...
def main(public_path, private_path):
    ...
    if public.exists() and any(public.iterdir()): raise FileExistsError("reaction v2 public directory must be new")
