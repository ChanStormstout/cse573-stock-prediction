...
def main(out):
 ...
 inputs=[B/'prepared/data.pkl',B/'prepared/manifest.json',B/'prepared/articles.npz',B/'prepared/embedding_inputs.json',W/'audit/news_index.pkl',B/'PROTOCOL.md',Path(__file__),O/'stock_four_hour_v2/text_vectorizer.py']
 hashes={str(p):sha(p) for p in inputs};dump(out/'sources.json',hashes)
 d=pd.read_pickle(inputs[0]);assert d.horizon.eq('4h').all();assert ((pd.to_datetime(d.end_utc,utc=True)-pd.to_datetime(d.start_utc,utc=True))==pd.Timedelta('4h')).all()
 for col in ['start_utc','end_utc','cutoff_utc']:d[col]=pd.to_datetime(d[col],utc=True)
 d['key']=d.symbol+'|'+d.start_utc.astype(str);d['month']=d.start_utc.dt.strftime('%Y-%m');d['day']=d.start_utc.dt.strftime('%Y-%m-%d');d['news_record_keys']=d.news_record_keys.fillna('');assert len(d)==1607 and not d.key.duplicated().any()
 ...
 dump(out/'status.json',dict(status='COMPLETE',horizon='4h',LR_fits=fitcount,seconds=time.monotonic()-t,source_hashes_unchanged=True,rows=1607,interpretation='Exploratory historical replay, not untouched test'))
