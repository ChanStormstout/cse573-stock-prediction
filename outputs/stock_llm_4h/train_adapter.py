...
def token_example(tok,r,label):
 ...
 tokens=p+a
 if len(tokens)>2048:raise ValueError(f"Refuse answer truncation: {r['id']} {len(tokens)}")
...
def loss(model,tokens,offset):
 x=mx.array(tokens)[None,:];logits=model(x[:,:-1]);target=x[:,1:]
 mask=mx.arange(1,len(tokens))>=offset
 return (nn.losses.cross_entropy(logits,target)*mask[None,:]).sum()/mask.sum()

def main():
 a=argparse.ArgumentParser();a.add_argument('--out',type=Path,required=True);a.add_argument('--data',type=Path,default=B/'data/pilot_v3');a.add_argument('--smoke',action='store_true');a.add_argument('--balance-events',action='store_true');a.add_argument('--seed',type=int,default=573);args=a.parse_args();out=new_run(args.out)
 ...
 config={'model':MODEL,'revision':REVISION,'fine_tune_type':'lora','num_layers':8,'lora_parameters':{'rank':8,'scale':16.,'dropout':.05,'keys':['self_attn.q_proj','self_attn.v_proj']},'seed':args.seed,'learning_rate':1e-4,'balance_events':args.balance_events,'micro_batch':1,'accumulation':8,'epochs':3 if not args.smoke else None,'smoke_micro_steps':32 if args.smoke else None,'max_seq_length':2048,'thinking':False,'assistant_only_loss':True,'train_ids':[r['id'] for r in train_labels],'valid_ids':[r['id'] for r in valid_labels],'labels_sha256':file_sha(args.data/'labels.jsonl'),'inputs_sha256':file_sha(args.data/'inputs.jsonl'),'source_sha256':file_sha(__file__),'common_sha256':file_sha(B/'common.py'),'quality_status':'ASSISTANT_PROVISIONAL_NOT_INDEPENDENT'}
 dump(out/'manifest.json',config);(out/'code_snapshot').mkdir();shutil.copy2(__file__,out/'code_snapshot/train_adapter.py');shutil.copy2(B/'common.py',out/'code_snapshot/common.py');mx.random.seed(args.seed);rng=np.random.default_rng(args.seed)
 ...
 for epoch in range(1,total_epochs+1):
  ...
  for idx in rng.permutation(len(train_labels)):
   label=train_labels[int(idx)];tokens,offset,_=examples[label['id']];tick=time.time();l,g=value_grad(model,tokens,offset);mx.eval(l,g)
   ...
   logs.append({'micro_step':steps,'epoch':epoch,'id':label['id'],'loss':value,'loss_weight':weight,'seconds':time.time()-tick,'optimizer_updates':updates,'peak_gb':mx.get_peak_memory()/1e9});write_rows(out/'training.jsonl',logs)
