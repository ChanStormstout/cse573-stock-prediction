...
for name,h in R['script_hashes'].items():
 current=B/name;archived=B.parent/'stock_review_fixes/original_code/stock_temporal'/name
 if hashlib.sha256(current.read_bytes()).hexdigest()!=h:
  assert archived.exists() and hashlib.sha256(archived.read_bytes()).hexdigest()==h, 'Historical source mismatch: '+name
  print('Historical code verified against preserved pre-review source:',name)
for s in ['AAPL','AMZN']:
