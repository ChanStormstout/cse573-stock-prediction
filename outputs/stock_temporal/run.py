...
sys.path.insert(0,str(B.parent/'stock_review_fixes'));from guards import verify_files
prepared=json.loads((OUT/'data_summary.json').read_text())
if prepared.get('status')!='prepared' or not prepared.get('artifact_hashes') or not prepared.get('preparation_inputs'):raise RuntimeError('Legacy/incomplete preparation: preserve it; prepare a new experiment directory.')
verify_files(OUT,prepared['artifact_hashes']);verify_files(B.parent,prepared['preparation_inputs'])
if any(OUT.rglob('*.pt')) or any(OUT.rglob('*.joblib')) or (OUT/'training_started.json').exists():raise RuntimeError('Partial training artifacts: use a new experiment version.')
(OUT/'training_started.json').write_text(json.dumps({'prepared_summary_sha256':hashlib.sha256((OUT/'data_summary.json').read_bytes()).hexdigest()}))
