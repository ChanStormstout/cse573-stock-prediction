...
B=Path(__file__).resolve().parent;OUT=B/'results'
sys.path.insert(0,str(B.parent/'stock_review_fixes'));from guards import require_empty,sha
require_empty(OUT);OUT.mkdir(exist_ok=True)
(OUT/'preparing.json').write_text(json.dumps({'status':'preparing','note':'Partial preparations must use a new directory, not overwrite.'}))
base=json.loads((B.parent/'stock_baseline/results/results.json').read_text())
...
manifest['artifact_hashes']={str(p.relative_to(OUT)):sha(p) for s in ['AAPL','AMZN'] for p in [OUT/s/'data.npz',OUT/s/'manifest.csv']}
manifest['preparation_inputs']={str(p.relative_to(B.parent)):sha(p) for p in [B.parent/'stock_baseline/results/results.json',B/'protocol.json',B/'prepare.py',B/'run.py',B/'models.py']+[B.parent/f'stock_finbert/results/{s}/development_features.csv' for s in ['AAPL','AMZN']]}
