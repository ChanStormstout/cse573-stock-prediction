"""Finite fixed-C attribution and frozen foundation-encoder comparisons."""
...
W=ROOT/'work/stock-data/foundation_4h'; PRIVATE=W/'v1'; OUT=HERE/'v1'; SOURCE=ROOT/'work/stock-data/paper_methods_4h/v1'
...
def init():
 ...
 sources=[SOURCE/'inputs.pkl',SOURCE/'aggregates.npz',ROOT/'outputs/stock_integrated_4h/prepared/articles.npz',HERE/'PRE_REGISTRATION.md']
...
def train_text(d,emb,ids,means,events,specs,stage):
 ...
 result=dict(fits=fits,choices=choices,code_sha256=sha(Path(__file__)),protocol_sha256=sha(HERE/'PRE_REGISTRATION.md'));dump(OUT/f'{stage}_training.json',result);dump(PRIVATE/f'{stage}_complete.json',{'fits':len(fits)});pd.DataFrame(cv).to_csv(OUT/f'{stage}_cv.csv',index=False)
 return d
