...
const REF='/Users/victor/.codex/plugins/cache/openai-curated-remote/openai-templates/0.1.1/skills/artifact-template-simple-light-mode/assets/reference.pptx';
...
'Classical methods\nOur verified word-feature models do not consistently beat the full-text reference across both stocks and periods.',
'Working hypothesis\nThe missing information may be the target company’s change relative to prior reports.'
],`These are distinct findings, not equivalent comparisons. Modern v2 used matched token pooling. Event full-fact F1 is a provisional extraction metric, not stock BA and not independent human gold. V10 is a separate classical experiment; the statement is its descriptive all-cell finding. The last panel is a hypothesis. Sources: ${src('outputs/stock_foundation_4h/v2/REPORT.md')}; ${src('outputs/stock_finbert_event_adapter_4h/v1/REPORT.md')}; ${src('outputs/stock_priorwork_repro/v10/CLASSICAL_LANE_FINAL_SUMMARY.md')}.`);
cols('The same negative wording can mean different things',[
'Prior report\nAn analyst cuts a target price from $200 to $180.\n\nNew article A\nAnother publisher repeats the same $180 target.\n\nCandidate relation: repeat.',
'Prior report\nThe target price is already $180.\n\nNew article B\nThe analyst cuts it again, to $160.\n\nCandidate relation: new change.'
...
table(s,[['Candidate','Additional information','What it tests'],['Reference','Price + FinBERT only','Starting forecast'],['+ History metadata','Coverage and history age','More context alone'],['+ Current facts','Company actions and values','Better article meaning'],['+ Fact changes','Changes relative to past reports','Value of new information']],190);
note(s,'Success requires repeatable gains on both stocks and reliable probabilities, then confirmation in a new period.');
cols('Monday discussion and the next decision',[
'Completed foundation\nA reproducible price + FinBERT model.\nChronological classical comparisons.\nReal event-adapter training.\nA frozen 515-card entity review pack.',
'Proposed next step\nValidate company links and pair relations before training the correction.\n\nFaculty feedback\nIs relative news change the right primary contribution for this course project?'
],`The review pack is a prerequisite, not a completed semantic validation. No independent entity review has passed yet. The broader 66-company external-data lane remains separate from the AAPL/AMZN course task and has no predictive result. This meeting can agree on a narrow contribution and data scope. Ask whether to prioritize a careful negative result plus clear mechanism test over a broad new model search. Sources: ${src('outputs/stock_ecni_e1rv2_gpt_review/GPT_REVIEW_README.md')} and ${src('docs/CURRENT_STATUS.md')}.`);
s=blank('Appendix: price + FinBERT across periods',`Saved canonical F2 values, no retraining. Brier is mean squared probability error; lower is better. A constant 0.5 forecast has Brier 0.25. BA=50% is a chance benchmark, not a proof of statistical significance. Source: ${src('outputs/stock_finbert_event_adapter_4h/v1/REPORT.md')}.`);
table(s,[['Period','AAPL BA','AAPL Brier','AMZN BA','AMZN Brier'],['Earlier monthly forecasts','50.02%','0.2602','48.14%','0.2749'],['Development','54.13%','0.2803','46.29%','0.2977'],['Later','56.71%','0.2856','54.27%','0.2617']],220);note(s,'BA balances up/down recall. Brier measures probability error (lower is better). Historical results.');
four('Appendix: what the evaluation supports',[
'Temporal split\nRandomly mixing historical periods answers a different question from forecasting the future.',
'Confirmed repair\nSome prior implementations required time-safety or comparison corrections. Their original results remain historical.',
'Pretraining exposure\nA modern encoder on 2018 text is a retrospective study. Downstream chronology cannot rule out prior exposure.',
'Claim boundary\nWe report retrospective evidence. A new untouched period is needed to confirm a generalization gain.'
],`Use precise, respectful language in the meeting. Distinguish confirmed implementation leakage, repeated development-set exposure, different evaluation protocols and uncertain pretraining overlap. Source: ${src('docs/METHOD_VALIDITY_AUDIT.md')}; ${src('docs/EXPERIMENT_CORRECTIONS.md')}; ${src('outputs/stock_priorwork_repro/v10/CLASSICAL_LANE_FINAL_SUMMARY.md')}. We have not conducted an independent paper-wide leakage audit for this deck.`);
cols('Appendix: sources and method status',[
...
// Numeric ambitions are explicitly distinct from measured or forecast performance.
s=blank('Evaluation target: the new pipeline has not yet run',`This slide states the user's desired research goal, not a forecast, empirical result, confidence interval, or registered promotion gate. No statistical basis supports forecasting 60% BA for the proposed method. The complete relative-news-change plus fixed price/FinBERT correction has not run. Keep actual results as NOT RUN. Existing extraction F1 and historical FinBERT BA cannot substitute for this result. Any future experiment needs the authorized preregistration and chronological matched evaluation. This presentation does not change an experimental protocol or authorize a paused lane.`);
text(s,'RESEARCH GOAL ONLY • NOT A PERFORMANCE FORECAST',42,159,1196,48,23,TEAL,true);
table(s,[['Company','Desired BA target','Measured new-pipeline BA'],['Apple','At least 60%','NOT RUN'],['Amazon','At least 60%','NOT RUN']],240);
text(s,'What would support an improvement claim?',42,444,1190,44,27,NAVY,true);
text(s,'Beat the unchanged reference on matched windows; report both companies and periods.\nConfirm any historical gain in a new, untouched period.',42,498,1180,78,25,INK);
note(s,'The 60% figure is an aspiration. No experiment or evidence currently establishes that this method will reach it.');
made.splice(13,0,made.pop());plans.splice(13,0,plans.pop());
...
const out=await finalizePresentation({workspaceDir:ROOT,candidatePath:path.join(BUILD,'candidate.pptx'),finalPath:path.join(OUT,'CSE573_News_Context_Faculty_Meeting_v9.pptx'),pythonExecutable:'/Users/victor/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3',integrityValidatorPath:path.join(SKILL,'container_tools/inspect_presentation_package_integrity.py'),layoutValidatorPath:path.join(SKILL,'container_tools/inspect_presentation_layout_geometry.py'),layoutArgs:['--expected-slide-size-emu','12192000,6858000','--validate-bullet-geometry','--validate-heading-fit','--require-native-table-slide','5','--require-native-table-slide','6','--require-native-table-slide','13','--require-native-table-slide','14','--require-native-table-slide','16'],fontPolicy:{basis:'reference',families:[FONT],referencePath:REF,referenceSha256:crypto.createHash('sha256').update(await fs.readFile(REF)).digest('hex')},explicitTotalSlideCount:18,requiredNativeTableOwnerSlides:[5,6,13,14,16],verifyArtifactToolImport:true,receiptPath:path.join(BUILD,'validation_v9.json')});
