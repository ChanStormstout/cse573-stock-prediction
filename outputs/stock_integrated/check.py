...
assert p[p.phase=='frozen'].groupby('symbol').size().to_dict()=={'AAPL':616,'AMZN':617}
assert pd.read_pickle(OUT/'inputs.pkl').shape[0]==3233
for path,h in json.loads((OUT/'sources.json').read_text()).items():assert sha(path)==h
print('PASS: 152 fit manifests and hashes; past-only C/weight selection recomputed; fusion probabilities reproduce; exact no-news price fallback; 3233 input keys and 1233 frozen replay windows; original input fingerprints unchanged.')
