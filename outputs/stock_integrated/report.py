...
def main():
 ...
 verification=[]
 inputs=pd.read_pickle(OUT/'inputs.pkl');z=np.load(O/'stock_comprehensive/runs/v1/M08/articles.npz');keys=z['keys'];emb=z['embeddings'].astype(np.float64)
 fp=json.loads((O/'stock_comprehensive/runs/v1/M08/embedding_fingerprint.json').read_text())
 for path,h in fp.items():
  candidate=Path(path)
  if candidate.name in ['articles.npz','embedding_inputs.json']:assert sha(candidate)==h
 for sym,d in inputs[inputs.month>='2018-09'].groupby('symbol'):
  ...
  joblib.dump(dict(symbol=sym,models=models,choice=c,embedding_sha256=sha(O/'stock_comprehensive/runs/v1/M08/articles.npz')),OUT/f'{sym}_system.joblib');verification.append(dict(symbol=sym,end_to_end_reload_error=error,n=len(d)))
 ...
 report=f'''# 完整组合流水线与传统新闻 baseline

## 这次实际完成了什么
四分支训练 → 过去样本外预测 → 过去数据选融合权重 → 冻结完整系统 → 开发/后续历史回放 → 消融/案例/区间。完成 {status['LR_fits']} 次成功LR训练，运行 {status['seconds']:.1f} 秒。另保留 v1 浮点重载检查中断记录，未用它挑分数。
