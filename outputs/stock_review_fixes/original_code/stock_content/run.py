...
if cache.exists() and meta.exists() and json.loads(meta.read_text())['input_sha256']==digest:pairs=pd.read_csv(cache)
else:
 ...
 pairs=pd.DataFrame(rows);pairs.to_csv(cache,index=False);meta.write_text(json.dumps(dict(input_sha256=digest,model='ProsusAI/finbert',revision=kw['revision'],device=device,n_unique_new_text=len(strings),elapsed_seconds=time.time()-start),indent=2))
