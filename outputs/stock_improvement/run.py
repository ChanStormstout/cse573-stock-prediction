...
if (OUT/'results.json').exists():raise RuntimeError('Completed E04: preserve historical results and use a new experiment version.')
...
embpath=ROOT/'finbert_embeddings_v2.npz';embmeta=ROOT/'finbert_embeddings_v2.json'
expected=dict(ordered_keys_and_titles=probs.reset_index()[['record_key','title']].fillna('').to_dict('records'),upstream_probabilities_sha256=sha(B.parent/'stock_finbert/results/article_probabilities.csv'),upstream_inference_sha256=sha(B.parent/'stock_finbert/results/inference.json'),model='ProsusAI/finbert',revision='4556d13015211d73dccd3fdd39d39232506f3e43',max_length=256,truncation=True,padding='longest',pooling='last hidden token mean excluding special and padding tokens; window article mean; fold-trained PCA16',torch=torch.__version__,transformers=package_version('transformers'),tokenizers=package_version('tokenizers'),dtype='float32',device='mps' if torch.backends.mps.is_available() else 'cpu',encoder_script_sha256=sha(__file__))
if not cache_readable(embpath,embmeta,expected):
 ...
 embmeta.write_text(json.dumps(cache_metadata(embpath,expected),indent=2))
