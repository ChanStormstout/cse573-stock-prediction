...
def run(root,out):
    if out.exists() and any(out.iterdir()):raise RuntimeError('Preserve legacy E02 outputs. New comparisons must use stock_robust training-only selection.')
    ...
    summary={'protocol':{'target':'full regular-session hour close > open; flat hours excluded','cutoff':'5 minutes before target start',
        ...
        'C_grid':[.01,.1,1.0],'selection':'LEGACY DEVELOPMENT ONLY: validation balanced accuracy, then lower Brier; use stock_robust for unified comparisons','test_evaluated':False,
