...
## 2026-09-19 — ECNI Stage E1R-V2 semantic entity and SEC coverage audit

**Why:** External review identified that E1R verified deterministic rule support but did not measure semantic entity precision. The `nasdaq` alias can denote NASDAQ, Inc. or a venue/market, while NDAQ had full training-day coverage and 255,784 training-period high-confidence edges.

**Preregistration and execution:** The semantic relations, sampling rules, precision gates, rule-level repair policy, foreign-filer policy, and outcome prohibitions were pushed first at `8f69790578820edb146565a3c6c487c9fbeac04d`. The complete 10.8-million-edge ledger was then aggregated into 3,129 alias/CIK rows. Raw articles were scanned only to recover private blinded review context and publisher-name overlap; no semantic label or market outcome was assigned.

**Review pack:** The frozen private pack has 515 unique cards: 330 panel-base cards covering every selected company exactly five times, 100 NDAQ sentinel cards, and 100 highest-risk-alias cards before overlap. The public manifest stores card IDs and the private SHA-256, not copyrighted text. Independent labels received = 0; therefore semantic precision and the Wilson gate remain unevaluated. Mechanical diagnostics flag 1,606 aliases, including high-volume single-token aliases such as `nasdaq`, `index`, `strategy`, `cheap`, `joint`, and `match`; flags are not semantic verdicts.
...
Semantic preparation runtime note: before any semantic encoder call or classifier fit, replaced repeated linear prefix tokenization with bounded prefix search under the same complete-paragraph/common-512-token contract. The initial empty output directory and log remain private; initial code is retained in f7fbae1. Completed input audit: 5,226 article-target pairs, 30,448 paragraphs, 31,059 common chunks. Independent raw-source hash/offset and full word-coverage verification PASS. FinBERT encoding is running; no semantic predictive result yet. Registered a fit-free FULL+existing-title fusion control before semantic results, to avoid automatically attributing mixture gains to longer input. Its selection remains inner-only and its limited matching caveat is explicit.

FinBERT paragraph encoding completed on MPS in 1,100.83 seconds with zero trainable encoder parameters/gradients. Fit-free preflight found and removed repeated lazy-array decompression before the first semantic classifier fit; materialized vectors are exactly equal to stored arrays. All 240 split/base-OOF checks pass. FinBERT semantic heads completed 600 fits and 60 selected checkpoints while Modern encodes. An exclusive trainer lock and complete-block hashes prevent duplicate fitting when the queued full invocation resumes. Predictive body scores remain unreported pending verification; no C/weight/features changed. See FULL_SEMANTICS_4H_HANDOFF.md.

## 2026-09-20 — FULL semantic finite extension complete

**Actual work:** FinBERT and Modern each encoded all 31,059 common chunks; Modern took 1395.37 seconds. 1,620 traditional top-level estimator fits and 1,200 semantic-head fits completed; 180 + 120 selected models independently replayed. SVM calibration has additional internal fitting. The full trainer validated/skipped the completed FinBERT blocks and fitted only the 60 remaining Modern blocks.

**Verification:** source/offset/word-coverage PASS; canonical fold and FULL-probability parity PASS; exact embedding reaggregation error 0 for both encoders; 9,642 body probabilities replayed with max error 4.44e-16; no-evidence fallback mismatches 0. No encoder gradients or learned stacker coefficients. All three split seeds reported, no best-seed selection.

**Observed:** FULL BA 67.57/60.28%; TF-IDF SVM 69.49/60.12%; FULL + FinBERT body 67.19/59.75%; FULL + Modern body 67.51/59.78% (AAPL/AMZN). Body variants lower Brier but do not improve mean BA. All descriptive day/five-day paired intervals for body BA increments include zero. Body evidence covers 99.25% of AAPL and 51.24% of AMZN windows; original article membership was unchanged.
...
## 2026-09-20 — Representation diagnostics preregistered and training

Owner authorized the ordered finite plan after review of the Pro suggestion. New lane: stock_representation_4h/v1. First isolate frequency, negation, numeric expressions and stemming under exactly historical cleaning, articles, nested splits and SVM C selection. Second run the same strong SVM with association-group outer/inner/calibration isolation. Third compare linear mean, map-after-mean and map-before-mean on frozen target-paragraph chunks with the same word/price LR head. No new encoder, LLM continuation, price interaction, TabPFN grid or external lane execution.

Preregistration and immutable code/input seal saved before training. Independently audited original text parity for all 1,607 windows; chunk reaggregation error 3.11e-15; grouped calibration partitions are feasible. Protected manifest binds 28,780 existing files. All existing random periods remain exposed. Results require fit-free model replay, inner selection checks and exact price fallback; partial outer results are not interpreted.


## 2026-09-20 — Representation diagnostics complete and verified

Completed all three frozen stages: four single-operation lexical ablations (2,400 SVM containers), group-disjoint SVM including calibration (600 containers), and four matched word/price plus semantic aggregation LR branches (2,400 containers). Actual real classifier fitting: 9,000 SVM internals and 2,400 LR, plus 9,000 sigmoid calibrators. No encoder inference/training, LLM continuation or external lane changes.

Independent fit-free replay: all 5,400 containers, maximum probability error 3.33e-16; 9,000 calibration memberships checked; inner C selection reconstructed; held-out geometry independently rebuilt; exact no-news PRICE fallback. Raw reconstruction: 5,078 articles and all 1,607 old-text rows. All 28,780 protected files unchanged, source/base probability parity exact, all 11,400 classifiers below iteration limits. Nine tests include synthetic save/reload, cache corruption and held-out geometry isolation. A three-chunk synthetic fixture initially had an identity distance permutation; expanded only that fixture and independently confirmed all 240 actual partitions have positive finite bandwidths. Predictive code remained frozen.

Ordinary random BA AAPL/AMZN: control SVM 69.55/61.14; frequency 70.13/60.61; negation 69.59/61.36; numeric 68.69/61.59; no stemming 69.09/60.35. Frequency's AAPL mean gain comes from one seed while two lose; all lexical paired intervals include zero. Numeric tokens consume about 1,284/974 of 5,000 selected vocabulary slots. Source-checked cases include repeated holdings templates and portfolio/ETF numbers; these observations do not establish causal market effects. Some changed predictions also involve an inner-selected C change, so they are not pure evidence of understanding a word or number.
...
## 2026-09-20 — GPT Pro finite completion started

Owner requested implementation of the remaining main recommendations after the representation study; a subsequent pause for method discussion ended with explicit resume. New isolated lane `stock_pro_completion_4h/v1` retains all1,607 windows, three seeds and ten random outer folds. It must not be reported as past-to-future generalization.

Preregistered before real fits: true-adjacency bigrams and NB weighting; calibrated sum-kernel SVC with mean/order/price-state interactions; independent compact-table LR/RF/TabPFN with/without semantic features; genuine nested-crossfit PCA16 residual. The earlier study's main-effect budget gate does not block these separately authorized interactions or TabPFN. No new encoder, LLM or external-data execution. Frozen input audit:5,078 articles,1,212 windows with eligible bigrams,35,119 protected files. Five synthetic contracts pass. Local TabPFN compatibility is isolated in the existing runtime; no package mutation. Real lexical training has started; no unverified outcome is claimed here.

## 2026-09-21 — Slide 6 full-text architecture proposal

Owner requested a paper-style redraw using academic-figures-drawer. Rebuilt only live slide 6 with native editable shapes, four processing stages, separate title path, reporting/coverage and price lanes, and an illustrative repeated-target-price/additional-guidance example. Three vector review cycles plus native/import visual correction completed. Google Drive saved state verified; normalized before/after PPTX text comparison confirms only slide 6 changed, with 16 slides retained. Editable draw.io/SVG and builder/quality documents stored under teacher_meeting_20260921/fulltext_figure_v1.

Status is explicitly proposed full-text extension; existing title-system results are not attributed to it. No model execution, results modification, independent predictive verification, raw-data publication or resumed experiment occurred. Other concurrent project changes are excluded from this task commit.
