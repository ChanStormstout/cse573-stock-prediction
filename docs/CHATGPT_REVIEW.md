...
优先读 [stock_analogy_4h](../outputs/stock_analogy_4h/README.md) 的两版登记、prompt逻辑、v2报告与输入案例审阅。
核查P2/P3是否只差历史已实现收益、案例是否只来自过去训练月份、当前标签是否未进入检索、
无案例/无新闻是否精确回退。特别关注AMZN案例覆盖与重复使用、目标公司和竞争对手角色混淆、
标题事件启发式的局限。V1因输入质量中止，其成本保留，不是按BA淘汰；没有新LLM微调。
区分“该有限检索实现无增益”和“所有历史类比预测都无效”。不能按后续成绩换股票专属赢家。
