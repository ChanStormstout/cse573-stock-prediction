...
`work/stock-data/nextgen_4h/` 保存修订数据清单、P0--P3密封prompt、正文精确span、
目标公司段落、在线重复组、逐次Qwen输出、LR权重、校准器、融合中间表、失败版本和
含原文的案例记录。公开的 `outputs/stock_nextgen_4h/` 只包含代码、协议、无原文的
数值预测/指标、聚合质量统计、模型及输入指纹和案例概述。

公开的 `LLM_SCORES.csv` 包含概率、token log-probability、prompt长度和运行成本，
不含prompt、标题、正文、URL或私有记录键。`DEDUP_AUDIT.csv`只发布聚合的报道和
独立来源数量。独立事件/段落人工验收尚未完成；助手工程检查不能替代它。
