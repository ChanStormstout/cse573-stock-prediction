...
A separately authorized preregistered round ran 208 new LR fits and frozen inference with Fin-ModernBERT and Chronos-2. The original Modern run included special tokens in its mean pool; the repaired v2 uses the same special-token-excluded pooling rule as canonical FinBERT, the same titles and training-only PCA16. The matched v2 reached AAPL development/later BA46.25%/54.52% and AMZN54.17%/54.97%; its training-period OOF BA was lower for AAPL and higher for AMZN, but it did not show a stable two-stock improvement. The v2 did not save the exact original promotion formula as a machine-readable gate, so this is not a formal pass/fail promotion claim. The older v1 numbers remain a historical unmatched diagnostic, not the clean encoder comparison.
...
## Further finite information and representation experiments

The [new round](../outputs/stock_goal60_4h/v1/REPORT.md) tested cross-stock features with LR/shallow boosting, a verified real-data-fine-tuned default TabPFN checkpoint (the historical v1 probe used n=1; the corrected v2 probe used n=8), original versus task-adapted dense FinBERT representations with matched linear/bilinear heads, a matched 8+8 title embedding control, and three-session historical-news state with already-realized price reactions. It used actual-system forward selection and reported development/later separately. No mechanism passed the predeclared weakest-stock improvement gate; no final combination was pursued. See the [TabPFN provenance and corrected probe](../outputs/stock_tabpfn_4h/v2/REPORT.md).

AMZN historical coverage filled all 95 later windows with no current four-hour news, yet H1/H2 later BA was 48.72%/48.67%. A1 dense LR produced later AAPL/AMZN 54.34%/46.65%. These results separate improved availability or event reading from validated predictive value. F2 with the new system-selection protocol reached later 55.14%/56.79% but AMZN development remained 49.17%. This is not evidence of stable 60% performance. All data periods remain exposed exploratory backtests. SPY/QQQ data was not available without Alpaca authentication and was not substituted or fabricated.
