...
| stock-specific reliability gate | `outputs/stock_specific_gate_4h/` | `UNVERIFIED_STOPPED_PENDING_EXTERNAL_REVIEW` | the one approved frozen run is preserved. A verifier-only scope repair enabled post-run verification; 17 checks pass, but independent advancement reconstruction fails because saved decimal serialization does not meet the verifier's `1e-12` comparison. No score is interpreted, reported as a method result, or used for tuning | repaired preflight or partial verifier success means the issued G0--G3 numbers are scientifically valid |
