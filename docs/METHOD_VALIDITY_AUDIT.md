...
| stock-specific reliability gate | `outputs/stock_specific_gate_4h/` | `PREREGISTERED_NOT_RUN` | repaired preflight verifies advantage, exact fallback, preprocessing, direction headroom, and R1/F1_new provenance; a distinct post-run verifier is implemented but no predictive result exists |
