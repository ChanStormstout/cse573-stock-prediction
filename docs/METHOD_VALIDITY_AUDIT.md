...
| dense price windows | 30-minute stride dense bars | dense sampling idea | 48 contiguous five-minute bars, reconstructed matched control when parity fails | MATCHED_COMPONENT_TEST | `outputs/stock_recency_dense_4h/build_dense_windows_v4.py`, `run_dense_v4.py` | VALID_WITH_CAVEAT | six-cell June–August matched comparison; no advancement | windows are independent/non-overlapping | Stage 2 complete; gate/parity verifier PASS |
| Fin-ModernBERT | modern financial encoder | Fin-ModernBERT | v2 frozen title encoder, special-token-excluded pooling, PCA/LR downstream | FOUNDATION_MODEL_PROBE | `outputs/stock_foundation_4h/encode_modern_v2.py`, `experiment_v2.py` | VALID_WITH_CAVEAT | matched frozen encoder probe has mixed exposed-period results and no promotion | reproduced Fin-ModernBERT stock forecasting or proved it ineffective | Stage 3 complete; no fusion |
